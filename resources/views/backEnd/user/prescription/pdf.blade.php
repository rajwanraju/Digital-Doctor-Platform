    ...
    <div id="container">
      <section id="memo">
        ...
        <div class="company-info">
          <div>
            <span>{{$info->name}}</span> <span>{{$info->designation}}</span>
          </div>
          <div>{{$info->Speciality}}</div>
          <div>{{$info->workPlace}}</div>
          <div>{{$info->graduation}},{{$info->graduationFrom}}</div>
          <div>{{$info->post_graduation}},{{$info->post_graduationFrom}}</div>
        </div>
      ...
      <section id="invoice-title-number">
        ...
        <span id="title">Mr.{{$user->name}}</span>
        <span id="number">{{$user->age}},{{$user->sex}}</span>

      </section>

      <div class="clearfix">Rx,</div>
<span id="title">Diseases Title:{{$report->diseases_title}}</span>
      <section id="items">

        <table >

          <tr>
            <th>Drug Name</th> <!-- Dummy cell for the row number and row commands -->
            <th>Drug Rules</th>
          </tr>

          <tr data-iterate="item">
            <td>{{$report->drug1}}</td>
            <td style="text-align:right">{{$report->rule1}}</td>
          </tr>
          @if($report->drug2 != null)
          <tr data-iterate="item">
            <td>{{$report->drug2}}</td>
            <td style="text-align:right">{{$report->rule2}}</td>
          </tr>
          @endif
          @if($report->drug3 != null)
          <tr data-iterate="item">
            <td>{{$report->drug3}}</td>
            <td style="text-align:right">{{$report->rule3}}</td>
          </tr>
          @endif
          @if($report->drug4 != null)
          <tr data-iterate="item">
            <td>{{$report->drug4}}</td>
            <td style="text-align:right">{{$report->rule4}}</td>
          </tr>
          @endif
          @if($report->drug5 != null)
          <tr data-iterate="item">
            <td>{{$report->drug5}}</td>
            <td style="text-align:right">{{$report->rule5}}</td>
          </tr>
          @endif
          @if($report->drug6 != null)
          <tr data-iterate="item">
            <td>{{$report->drug6}}</td>
            <td style="text-align:right">{{$report->rule6}}</td>
          </tr>
@endif
